		...
		try {
			...
			switch(command) {
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, '*TEXTO MUITO LONGO FDP*', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('*marque a pessoa que você quer chutar!*')
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('COMANDO *welcome* ATIVADO COM SUCESSO ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('COMANDO *welcome* DESATIVADO COM SUCESSO ✔️')
					} else {
